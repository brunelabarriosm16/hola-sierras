...
function LocalidadField({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  return <div className="space-y-2"><label className="text-sm font-medium text-slate-700">Localidad</label><select value={value} onChange={(event) => onChange(event.target.value)} className="w-full rounded-2xl border border-slate-200 px-4 py-3 outline-none transition focus:border-blue-400"><option value="">Sin definir</option>{LOCALIDADES.map((localidad) => <option key={localidad} value={localidad}>{localidad}</option>)}</select></div>
}
