...
export default async function InstitucionSharePage({
  ...
  const { data } = await supabaseServer
    .from("instituciones")
    .select("id, nombre, descripcion, premium_detalle, premium_galeria, premium_extra_titulo, premium_extra_detalle, premium_extra_galeria, premium_activo, direccion, localidad, telefono, web_url, instagram_url, facebook_url, foto, usa_whatsapp, estado, owner_email")
    .eq("id", Number(id))
    .maybeSingle()
  ...
  const [explicitRelatedEvents, legacyRelatedEvents] = await Promise.all([
    supabaseServer
      ...
      .eq("estado", "activo")
      .eq("related_entity_type", "institucion")
